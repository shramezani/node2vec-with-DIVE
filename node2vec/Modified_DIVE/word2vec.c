 ...
 *
 */
char train_file[MAX_STRING], output_file[MAX_STRING], pmi_file[MAX_STRING];
 ...
 */
long long vocab_max_size = 1000, vocab_size = 0, total_co_occurrence=0, layer1_size = 100;
real Z = 0;
...
long long calculate_total_co_occurrence() {
	long long result = 0;
	...
	////QUESTION sigma of all frequency calculates 2*total_co_ocr
...
bool* get_pmi_filtering(long long _vocab_size){

	FILE *myFile;
    long long index_i,index_j;
    char c;
    myFile = fopen(pmi_file, "r");

    //read file into array
    bool* pmi_array= malloc(sizeof(bool) * _vocab_size *_vocab_size);
    if (pmi_array == NULL) {
    ...
    while(1){
        		fscanf(myFile, "%lld%c%lld", &index_i,&c,&index_j);
			if(index_i == -1)
				break;
			pmi_array[index_i*vocab_size+index_j] = true;
			//printf("%lld ---- %lld -- %d",index_i,index_j,numberArray[index_i*vocab_size+index_j]);
			//temp = temp*numberArray[index_i*vocab_size+index_j];
...
void InitNet() {
  ...
  //https://www.quora.com/How-are-vectors-initialized-in-word2vec-algorithm

  long long a, b;
  unsigned long long next_random_syn0 = 1, next_random_syn1neg = 0.1;
  ...
  if (negative>0) {
    ...
    // Set all of the weights in the output layer randomely.
    for (a = 0; a < vocab_size; a++) for (b = 0; b < layer1_size; b++){
      next_random_syn1neg = next_random_syn1neg * (unsigned long long)25214903917 + 11;
      syn1neg[a * layer1_size + b] = (((next_random_syn1neg & 0xFFFF) / (real)65536)) / 10.0;
  ...
  for (a = 0; a < vocab_size; a++) for (b = 0; b < layer1_size; b++) {
    ...
    next_random_syn0 = next_random_syn0 * (unsigned long long)25214903917 + 11;
    syn0[a * layer1_size + b] = (((next_random_syn0 & 0xFFFF) / (real)65536)) / 10.0;
...
void *TrainModelThread(void *id) {
  ...
  real f, g;
  clock_t now;
  real skip_gram_negative_sample_weight_context = 0;
  ...
  while (1) {
     ...
     */
      ...
      // the index relative to the beginning of the sentence.
		...
		/***SHIVA***/
			skip_gram_negative_sample_weight_context = ((0.3 * Z)/(vocab[last_word].cn*2));
	        if (negative > 0) for (d = 0; d < negative + 1; d++) {
	          ...
	          //if (pmi_array[last_word*vocab_size+target]==true ) {
		  
	          if (label == 0 ){
				  /*** context_D ***/
				  g= g*skip_gram_negative_sample_weight_context ;// window is deleted from Z and freq*window
				  // target_D //
				  //g= g *((0.3 * Z)/(vocab[target].cn*window*2));
				  // using objective function's weight //
				  // target_over_V//
				  //g= g *((0.3*vocab[target].cn *window*2)/(vocab_size));
				  // context_over_V //
				  //g= g *((0.3*vocab[last_word].cn *window*2)/(vocab_size));
					...
					{ 
							...
							// 0.3 is 1.5/5    5 is negative sampling ratio //

							//co occur is  29193859
							// context_D //
							neu1e[c] += g * syn1neg[c + l2];
							// target_D //
					  		//neu1e[c] += g * syn1neg[c + l2]* ((0.3 * Z)/(vocab[target].cn*window*2));
							// using objective function's weight //
							// target_over_V//
							//neu1e[c] += g * syn1neg[c + l2]* ((0.3*vocab[target].cn *window*2)/(vocab_size));
							// context_over_V //
						    //neu1e[c] += g * syn1neg[c + l2]*((0.3*vocab[last_word].cn *window*2)/(vocab_size));
					...
					for (c = 0; c < layer1_size; c++)
					{
						...
						//target_D //
						//syn1neg[c + l2] += g * syn0[c + l1] * ((0.3 * Z)/(vocab[target].cn*window*2));
						// using objective function's weight //
						// target_over_V//
						//syn1neg[c + l2] += g * syn0[c + l1] * ((0.3*vocab[target].cn *window*2)/(vocab_size));
						//context_over_V //
						//syn1neg[c + l2] += g * syn0[c + l1] * ((0.3*vocab[last_word].cn *window*2)/(vocab_size));
...
void TrainModel() {
  ...
  /***SHIVA***/
  total_co_occurrence = calculate_total_co_occurrence();
  printf("TOAL CO OCR IS EQUAL TO =================== %lld ",total_co_occurrence);
  //pmi_array = get_pmi_filtering(vocab_size);
...
int main(int argc, char **argv) {
  ...
  if ((i = ArgPos((char *)"-pmi-file", argc, argv)) > 0) strcpy(pmi_file, argv[i + 1]);
